...
说明：当前解释器镜像以 CPython 3.8-3.12 为主，默认不覆盖 3.13/3.14。

## 1. 外网机首次全量下载

```bash
./target/release/pip-mirror sync-full -c pip-mirror.toml
```
...
./target/release/pip-mirror serve -c pip-mirror.toml
```
...
```bash
./target/release/pip-mirror sync -c pip-mirror.toml
```
...
```bash
./target/release/pip-mirror import-incremental \
...
./target/release/pip-mirror import-incremental <incremental.tar.gz> -c pip-mirror.toml
